...
EXEC @ReturnCode = msdb.dbo.sp_add_job @job_name=N'DeleteExpiredVIP', 
    @enabled=1, 
...
-- ایجاد Step برای Job
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'DeleteExpiredVIPUsers', 
    @step_id=1, 
    ...
    @command=N'
                ...
                WHERE id IN (SELECT id FROM VIP_CLIENT WHERE Subscription_expiration_time <= GETDATE())
                AND number BETWEEN (id * 100 + 2) AND (id * 100 + 5);
